...
public class InfernalWarPredicateProvider {
    ...
    private static void registerCrossbow(InfernalWarCrossbow crossbow){
        ModelPredicateProviderRegistry.register(crossbow, new Identifier("pull"), (ItemStack, ClientWorld, LivingEntity, seed) -> {
            ...
            return (float)(ItemStack.getMaxUseTime() - LivingEntity.getItemUseTimeLeft()) / (float)InfernalWarCrossbow.getPullTime(ItemStack);
        });
        ...
        ModelPredicateProviderRegistry.register(crossbow, new Identifier("firework"), (stack, world, entity, seed) -> entity != null && InfernalWarCrossbow.isCharged(stack) && InfernalWarCrossbow.hasProjectile(stack, Items.FIREWORK_ROCKET) ? 1.0f : 0.0f);
    }
}
