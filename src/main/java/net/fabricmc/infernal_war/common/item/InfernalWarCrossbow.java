...
public class InfernalWarCrossbow extends RangedWeaponItem implements Vanishable {
    private static final String CHARGED_KEY = "Charged";
    private static final String CHARGED_PROJECTILES_KEY = "ChargedProjectiles";
    private int DEFAULT_PULL_TIME;
    public int RANGE;
    ...
    private static boolean loadProjectiles(LivingEntity shooter, ItemStack projectile) {
        ...
        for (int k = 0; k < j; ++k) {
            ...
            if (InfernalWarCrossbow.loadProjectile(shooter, projectile, itemStack, k > 0, bl)) continue;
            return false;
        }
        return true;
    }
    ...
    private static boolean loadProjectile(LivingEntity shooter, ItemStack crossbow, ItemStack projectile, boolean simulated, boolean creative) {
        ...
        InfernalWarCrossbow.putProjectile(crossbow, itemStack);
        return true;
    }
    ...
    public static boolean hasProjectile(ItemStack crossbow, Item projectile) {
        return InfernalWarCrossbow.getProjectiles(crossbow).stream().anyMatch(s -> s.isOf(projectile));
    }
    ...
    private static void shoot(World world, LivingEntity shooter, Hand hand, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean creative, float speed, float divergence, float simulated) {
        ...
        if (bl) {
            ...
            projectileEntity = InfernalWarCrossbow.createArrow(world, shooter, crossbow, projectile);
    ...
    public static void shootAll(World world, LivingEntity entity, Hand hand, ItemStack stack, float speed, float divergence) {
        List<ItemStack> list = InfernalWarCrossbow.getProjectiles(stack);
        float[] fs = InfernalWarCrossbow.getSoundPitches(entity.getRandom());
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (i == 0) {
                InfernalWarCrossbow.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, 0.0f);
                continue;
            }
            if (i == 1) {
                InfernalWarCrossbow.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, -10.0f);
                continue;
            }
            if (i != 2) continue;
            InfernalWarCrossbow.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, 10.0f);
        }
        InfernalWarCrossbow.postShoot(world, entity, stack);
    }

    private static float[] getSoundPitches(Random random) {
        boolean bl = random.nextBoolean();
        return new float[]{1.0f, InfernalWarCrossbow.getSoundPitch(bl, random), InfernalWarCrossbow.getSoundPitch(!bl, random)};
    }
    ...
    private static void postShoot(World world, LivingEntity entity, ItemStack stack) {
        ...
        InfernalWarCrossbow.clearProjectiles(stack);
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        List<ItemStack> list = InfernalWarCrossbow.getProjectiles(stack);
        if (!InfernalWarCrossbow.isCharged(stack) || list.isEmpty()) {
            return;
        }
